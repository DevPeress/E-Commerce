import { email } from "zod";
import { VerificarSenha, Criptografar } from "../lib/bcrypt";
import db from "../lib/mysql";
import logger from "../lib/pino";
import { LoginInput, RecInput, RegisterInput } from "../schemas/authSchemas";
import { Cargo, Cargo_Id, Login, Register } from "../types/auth";

export const AuthDB = {
  async getByEmail(
    email: string
  ): Promise<{ success: boolean; data?: Register[]; error?: string }> {
    try {
      const [rows] = await db.query<Register[]>(
        "SELECT email FROM Contas WHERE email = ? LIMIT 1",
        [email]
      );
      logger.info("Iniciou a procura do registro do email: " + email);

      if (rows.length === 0) return { success: false, error: "Não foi localizado o email!" };
      return { success: true, data: rows };
    } catch (err) {
      logger.error("Register GetByEmail: " + err);
      console.error("Register GetByEmail: ", err);
      return { success: false, error: "Erro ao localizar email!" };
    }
  },

  async postRegister(data: RegisterInput): Promise<{ success: boolean; error?: string }> {
    try {
      const dados = await AuthDB.getByEmail(data.email);
      if (dados.success) return { success: false, error: "Email possui uma conta já!" };

      const senhaProtegida: string = await Criptografar(data.senha);
      await db.execute("INSERT INTO Contas(nome,email,senha,cpf,idade,cep) VALUES(?,?,?,?,?,?)", [
        data.nome,
        data.email,
        senhaProtegida,
        data.cpf,
        data.idade,
        data.cep,
      ]);

      logger.info("Cadastro do email: " + email + "na empresa!");
      return { success: true };
    } catch (err) {
      logger.error("Register PostRegister: " + err);
      console.error("Register PostRegister: ", err);
      return { success: false, error: "Erro ao criar conta!" };
    }
  },

  async getLogin(
    dados: LoginInput
  ): Promise<{ success: boolean; data?: Login[]; error?: string; cargo?: string }> {
    try {
      const [rows] = await db.query<Login[]>(
        "SELECT email, senha FROM Contas WHERE email = ? LIMIT 1",
        [dados.email]
      );
      logger.info("Iniciou a procura do email: " + dados.email);
      if (rows.length === 0) return { success: false, error: "Email ou senha estão incorretos!" };

      const senhaCorreta: boolean = await VerificarSenha(dados.senha, rows[0].senha);
      if (!senhaCorreta) {
        logger.info("Digitou a senha errada!");
        return { success: false, error: "Email ou senha estão incorretos!" };
      }

      logger.info("Iniciou o login!");

      const [info] = await db.query<Cargo_Id[]>(
        "SELECT cargo_id FROM Funcionarios WHERE email = ?",
        [dados.email]
      );

      if (info.length === 0) return { success: true, data: rows, cargo: "Cliente" };
      const [cargo] = await db.query<Cargo[]>("SELECT cargo FROM Cargos WHERE id = ?", [
        info[0].cargo_id,
      ]);

      if (cargo.length === 0) return { success: true, data: rows, cargo: "Cliente" };

      return { success: true, data: rows, cargo: cargo[0].cargo };
    } catch (err) {
      logger.error("Login GetLogin: " + err);
      console.error("Login GetLogin: ", err);
      return { success: false, error: "Erro ao realizar o login!" };
    }
  },

  async putLogin(dados: RecInput): Promise<{ success: boolean; error?: string }> {
    try {
      const data = await AuthDB.getByEmail(dados.email);
      if (!data.success || !data.data) return { success: false, error: data.error };

      const verify: boolean = await VerificarSenha(data.data[0].senha, dados.senha);
      if (!verify) return { success: false, error: "Senha atual está errada!" };

      const senhaCriptografada = Criptografar(dados.senhaNova);

      await db.execute("UPDATE INTO Contas SET senha = ? WHERE email = ?", [
        senhaCriptografada,
        dados.email,
      ]);
      return { success: true };
    } catch (err) {
      logger.error("Recuperação: " + err);
      console.error("Recuperação: ", err);
      return { success: false, error: "Erro ao realizar a recuperação de senha!" };
    }
  },
};
