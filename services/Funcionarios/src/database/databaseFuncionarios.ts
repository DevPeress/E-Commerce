...
export const funcionariosDB = {
  ...
  async putFuncionario(
    ...
    try {
      ...
      logger.info(
        "Atualizou o funcionario de nome: " +
          dados.data[0].nome +
          " Tipo: " +
          tipo +
          " Valor: " +
  ...
  async putAll(ajustes: Usuarios[]): Promise<{ sucess: boolean; error?: string }> {
    ...
    try {
      ...
      await Promise.all(
        ajustes.map((infos) =>
          connection.execute(
            "UPDATE Funcionarios SET nome = ?, email = ?, cpf = ?, idade = ?, cep = ?, cargo = ? WHERE id = ?",
            [infos.nome, infos.email, infos.cpf, infos.idade, infos.cep, infos.cargo, infos.id]
          )
        )
      );
